...
改名在 git 眼中是「刪掉一個檔 + 新增一個檔」。雖然 `git log --follow` 追得回去，
但 `git blame`、GitHub 網頁上的 history、以及大部分 diff 工具都會在版號交界處斷掉。
主程式有四千多行，檔名一路從 `V0.43`、`V0.50` 改到 `V0.52`，歷史已經被切成三段。
...
動手之前會檢查四件事，任何一項不過就停下來，什麼都不改：

| 檢查 | 不過的原因 |
|---|---|
| 分支是 `main` | 從功能分支發版，做出來的版本會少東西 |
| 工作區乾淨 | 待會要 commit `VERSION_STR`，夾帶到別的改動就洗不掉了 |
| 與遠端沒有落後或分歧 | tag 會打在少東西的 commit 上，而且 `git push` 也會被拒 |
| tag 尚未存在 | 覆蓋既有 tag 會讓已經下載過的人拿到對不上的版本 |
